...
<constant definition> ::= <identifier> = <constant> <constant> ::= <unsigned number> | <sign> <unsigned number> | <constant identifier> | <sign> <constant identifier> | <string>
...
<function identifier> := <expression> <variable> ::= <entire variable> | <component variable> | <referenced variable>
...
def antlr(rule):
  ...
  for i in range(1, len(rule)):
    ...
    if rule[i] == ' ':
      ...
      continue
    if (isalpha(rule[i])):
      out += (rule[i] if not cap else rule[i].upper())
      cap = False
    ...
    while i < len(lines[1]):
      if (lines[1][i] == '<'):
        ...
        if lines[1][i] == '{':
          ...
          if i < len(lines[1]) - 1 and lines[1][i+1] == '.':
            print('DOT_DOT', end='')
            i = i + 1
          else:
            print('DOT', end='')
